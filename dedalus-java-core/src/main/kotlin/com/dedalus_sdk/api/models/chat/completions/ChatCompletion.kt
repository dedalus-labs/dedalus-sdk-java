...
private constructor(
     ...
     *   server responded with an unexpected value).
     */
    fun deferred(): Optional<List<DeferredCallResponse>> = deferred.getOptional("deferred")
     ...
     *   server responded with an unexpected value).
     */
    fun pendingTools(): Optional<List<PendingTool>> = pendingTools.getOptional("pending_tools")
     ...
     *   server responded with an unexpected value).
     */
    fun serverResults(): Optional<ServerResults> = serverResults.getOptional("server_results")
     ...
     *   server responded with an unexpected value).
     */
    fun turnsConsumed(): Optional<Long> = turnsConsumed.getOptional("turns_consumed")
    ...
    class Builder internal constructor() {
        ...
        fun deferred(deferred: Optional<List<DeferredCallResponse>>) =
            deferred(deferred.getOrNull())
        ...
        /** Alias for calling [Builder.pendingTools] with `pendingTools.orElse(null)`. */
        fun pendingTools(pendingTools: Optional<List<PendingTool>>) =
            pendingTools(pendingTools.getOrNull())
        ...
        /** Alias for calling [Builder.serverResults] with `serverResults.orElse(null)`. */
        fun serverResults(serverResults: Optional<ServerResults>) =
            serverResults(serverResults.getOrNull())
        ...
        /** Alias for calling [Builder.turnsConsumed] with `turnsConsumed.orElse(null)`. */
        fun turnsConsumed(turnsConsumed: Optional<Long>) = turnsConsumed(turnsConsumed.getOrNull())
    ...
    fun validate(): ChatCompletion = apply {
        ...
        correlationId()
        deferred().ifPresent { it.forEach { it.validate() } }
        mcpServerErrors().ifPresent { it.validate() }
        mcpToolResults().ifPresent { it.forEach { it.validate() } }
        pendingTools().ifPresent { it.forEach { it.validate() } }
        serverResults().ifPresent { it.validate() }
